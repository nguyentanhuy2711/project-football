...
//    double getPriceProduct(Long id);
    //duc
    @Query(value = "SELECT * FROM project_football_pitch.products where (status = 1 and quantity > 0) order by modifieddate DESC limit 8",nativeQuery = true)
    List<ProductsEntity> findLimitByDate();

    @Query(value = "SELECT * FROM project_football_pitch.products where (status = 1 and quantity > 0 and  name LIKE %:keyword% ) order by modifieddate DESC ",nativeQuery = true)
    List<ProductsEntity> findAllPagined(String keyword);

    @Query(value = "SELECT * FROM project_football_pitch.products WHERE id = :proId",nativeQuery = true)
    List<ProductsEntity> findOneByID(long proId);

    @Query(value = "SELECT * FROM project_football_pitch.products where (status = 1 and quantity > 0 and category_product_id = :cateId ) order by modifieddate DESC ",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "update project_football_pitch.products set quantity = (quantity-:quantity) where id = :id", nativeQuery = true)
